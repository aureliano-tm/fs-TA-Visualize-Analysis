...
import numpy as np
from numpy import exp, sqrt, pi, heaviside, power, log10, shape
import scipy
...
def quickfit_kinetics_v3(
    func, p0, df_2cols=None, df_1cols=None, xlimit=None, bounds=(-np.inf, np.inf), yaxistype="linear"
):
    if df_1cols is not None:
        df_2cols = mf.reshape_1colto2col(df_1cols)
    else:
        df_2cols = df_2cols

    num_set = int(df_2cols.shape[1] / 2)
    fittings = []
    for i in range(0, num_set):
        data = df_2cols.iloc[:, 2 * i : 2 * i + 2].dropna(axis=0).copy()
        x_raw = data.iloc[:, 0]
        y_raw = data.iloc[:, 1]
        wl = data.columns[1]
        if xlimit is not None:
        ...
        rss = np.sum((y_crop - y_fit) ** 2)
        r2 = 1 - (rss / tss)
        data_fit = pd.concat([data, df_fit], axis=1)
        data_fit_residual = pd.concat([data, df_fit, df_residual], axis=1)
...
def fit_fsta_kinetics(
    func, p0, df_2cols=None, df_1cols=None, bounds=(-np.inf, np.inf), yaxistype="linear"
):
    if df_1cols is not None:
        df_2cols = mf.reshape_1colto2col(df_1cols)
    else:
    ...
    for i in range(0, num_set):
        ...
        x_data = df_2col.iloc[:, 0]
        y_data = df_2col.iloc[:, 1]
        wavelength = df_2col.columns[1]
        # func = getattr(mm, func)
        popt, pcov = curve_fit(
            xdata=x_data,
            ydata=y_data,
        ...
        fit = pd.DataFrame({"time": x_data, f"{func.__name__}_fit": y_guess})
        r = pd.DataFrame({"time": x_data, "residual": residual})
        # mean square error
        mse = np.mean(r.iloc[:, -1] ** 2)
        # root mean square error
        ...
        fitting.append(fitting_results)

        # data_fit_residual = pd.DataFrame(
        #     {
        #         df_2col.columns[1]: y_data.tolist(),
        #         f"{func.__name__}_fit": y_guess.tolist(),
        #         "residual": residual.tolist(),
        #     },
        #     index=x_data,
        ...
        fig.add_trace(
            go.Scatter(x=x_data, y=y_guess, name=f"{func.__name__} Fit", showlegend=True),
            row=1,
            col=1,
        )
        fig.add_trace(
...
def gauss(x, sigma=0.1, mu=0, scale=1):
    y = exp(-0.5 * ((x - mu) ** 2) / sigma**2)
    y /= sigma * sqrt(2 * pi)
    return y * scale


def target_ST_ISC(times, pardf):
    # k0: S1 decay
    # k1: ISC
    # k2: T1 decay
...
def target_STLn_TEnT(times, pardf):
    # k0: S1 decay
    # k1: ISC
    # k2: T1 decay
    # k3: T1 EnT
...
def target_STLn_STEnT(times, pardf):
    # k0: S1 decay
    # k1: ISC
    # k2: T1 decay
    # k3: T1 EnT
...
if __name__ == "__main__":
    df_fsta = mins.load_fsta(filepath="/Users/tm/Documents/Programming/python/demo_data/fsta.csv")
    print(df_fsta)
    df_fsta_t = df_fsta.T
    print(df_fsta_t)