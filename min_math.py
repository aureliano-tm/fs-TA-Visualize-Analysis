...
def P13(times, pardf):
    ...
    for i in range(1, len(times)):  # iterate over all timepoints
        ...
        for j in range(int(sub_steps)):
            dc[0] = (
                -pardf["k0"] * dt * c_temp[0]
                - pardf["k4"] * dt * c_temp[0]
                - pardf["k2"] * dt * c_temp[0]
                + g[i] * dt
            )
            dc[1] = pardf["k0"] * dt * c_temp[0] - pardf["k1"] * dt * c_temp[1]
            dc[2] = pardf["k2"] * dt * c_temp[0] - pardf["k3"] * dt * c_temp[2]
            dc[3] = (
                pardf["k1"] * dt * c_temp[1]
                + pardf["k3"] * dt * c_temp[2]
                + pardf["k4"] * dt * c_temp[0]
            )
            for b in range(c.shape[1]):
                c_temp[b] = np.nanmax(
...
def P31(times, pardf):
    ...
    for i in range(1, len(times)):  # iterate over all timepoints
        ...
        for j in range(int(sub_steps)):
            dc[0] = (
                -pardf["k0"] * dt * c_temp[0] - pardf["k3"] * dt * c_temp[0] + g[i] * dt
            )
            dc[1] = pardf["k0"] * dt * c_temp[0] - pardf["k1"] * dt * c_temp[1]
...
def P33(times, pardf):
    ...
    for i in range(1, len(times)):  # iterate over all timepoints
        ...
        for j in range(int(sub_steps)):
            ...
            dc[4] = (
                +g[i] * dt - pardf["k3"] * dt * c_temp[1] - pardf["k2"] * dt * c_temp[2]
            )
            for b in range(c.shape[1]):
                c_temp[b] = np.nanmax(
                    [(c_temp[b] + float(dc[b])), 0.0]
...
def P41(times, pardf):
    ...
    for i in range(1, len(times)):  # iterate over all timepoints
        ...
        for j in range(int(sub_steps)):
            dc[0] = (
                -pardf["k0"] * dt * c_temp[0] - pardf["k4"] * dt * c_temp[0] + g[i] * dt
            )
            dc[1] = pardf["k0"] * dt * c_temp[0] - pardf["k1"] * dt * c_temp[1]
            dc[2] = (
                pardf["k1"] * dt * c_temp[1]
                + pardf["k4"] * dt * c_temp[0]
                - pardf["k2"] * dt * c_temp[2]
                - pardf["k5"] * dt * c_temp[2]
            )
