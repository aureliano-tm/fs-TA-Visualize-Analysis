...
def choose_files(initial_folder, file_types=[("Text files", "*.txt")]):
    root = tk.Tk()
    root.withdraw()
    filepaths = filedialog.askopenfilenames(
        initialdir=initial_folder, filetypes=file_types
    )
    return filepaths
...
def determine_baseline_v2(df_2cols, wl_limit):
    # df_2ol with only two columns
    # wl_limit = [lower_limit, upper_limit]
    chosen_data = choose_spectra_data(df_2cols, wl_limit)
    baseline = chosen_data.iloc[:, 1].mean()
    return baseline
...
#     e3cm1 = np.full_like(nm, np.nan, dtype=float)
    
#     # Calculate e3cm1 for non-zero values
#     e3cm1[~mask_zero] = 10**4 / nm[~mask_zero]
...
#     # Initialize output array with NaNs
#     nm = np.full_like(e3cm1, np.nan, dtype=float)

#     # Calculate nm for non-zero values
#     nm[~mask_zero] = 10**4 / e3cm1[~mask_zero]
...
def move_column_by_index(df, column_index, new_index):
    """
    Move a column in a DataFrame to a new position based on column indices.

    Parameters:
        df (DataFrame): The DataFrame containing the columns.
        column_index (int): The index of the column to move.
        new_index (int): The new index where the column should be moved.

    Returns:
        DataFrame: DataFrame with the column moved to the new position.
    """
    cols = df.columns.tolist()
    cols.insert(new_index, cols.pop(column_index))
    return df[cols]

def set_range_2coldf(df_2col, range):
    df_copy = deepcopy(df_2col)
    df_copy = df_copy.loc[(df_copy.iloc[:, 0] > range[0]) & (df_copy.iloc[:, 0] < range[1])]
    return df_copy


def rm_range_1coldf(df_1col, range):
    df_copy = deepcopy(df_1col)
    df_copy = df_copy.loc[(df_copy.index <= range[0]) | (df_copy.index >= range[1])]
    return df_copy


def scale_2coldf(df_2col, scale):
    df_copy = deepcopy(df_2col)
...
def determine_timedelay_label(xlimit): # unit in ps
    if xlimit[0][1] < 100:
        major_tick = list(
    ...
    elif xlimit[0][1] >= 10000:
        major_tick = list(
            set(generate_ticks_linearfield([xlimit[0][0], xlimit[0][1]], xlimit[1]))
            # | set([i for i in [100000, 1000000, 10000000, 100000000, 1000000000, 10000000000] if i > xlimit[0][1]])
        )
        minor_tick = list(
            set(generate_ticks_linearfield([xlimit[0][0], xlimit[0][1]], xlimit[2]))
            # | set([2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000])
...
def determine_ns_timedelay_label(xlimit): # unit in ns
    if xlimit[0][1] < 1000: # 1000 ns = 1 us
        major_tick = list(
            set(generate_ticks_linearfield([xlimit[0][0], xlimit[0][1]], xlimit[1]))
            | set([i for i in [5000, 10000] if i > xlimit[0][1]])
        )
        minor_tick = list(
            set(generate_ticks_linearfield([xlimit[0][0], xlimit[0][1]], xlimit[2]))
            # | set()
        )
    elif xlimit[0][1] >= 1000 and xlimit[0][1] < 1000000: # 1000000 ns = 1 ms
        major_tick = list(
            set(generate_ticks_linearfield([xlimit[0][0], xlimit[0][1]], xlimit[1]))
            | set([i for i in [1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000, 10000000000] if i > xlimit[0][1]])
        )
        minor_tick = list(
            set(generate_ticks_linearfield([xlimit[0][0], xlimit[0][1]], xlimit[2]))
            # | set()
        )
    elif xlimit[0][1] >= 1000000 and xlimit[0][1] < 1000000000: # 1000000000 ns = 1 s
        major_tick = list(
            set(generate_ticks_linearfield([xlimit[0][0], xlimit[0][1]], xlimit[1]))
            | set([i for i in [100, 500, 1000, 5000, 10000] if i > xlimit[0][1]])
        )
