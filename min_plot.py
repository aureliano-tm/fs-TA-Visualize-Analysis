...
# lnstent = ((0.17647058823529413, 0.44313725490196076, 0.7137254901960784), (0.8549019607843137, 0.21568627450980393, 0.19215686274509805), (0.9803921568627451, 0.9294117647058824, 0.3333333333333333), (0.33725490196078434, 0.6980392156862745, 0.4745098039215686), (0.4627450980392157, 0.30980392156862746, 0.396078431372549), (0.6, 0.6, 0.6), (0.0, 0.0, 0.0))
lnstent=ListedColormap(((0.17647058823529413, 0.44313725490196076, 0.7137254901960784), (0.8549019607843137, 0.21568627450980393, 0.19215686274509805), (0.9803921568627451, 0.9294117647058824, 0.3333333333333333), (0.33725490196078434, 0.6980392156862745, 0.4745098039215686), (0.4627450980392157, 0.30980392156862746, 0.396078431372549), (0.6, 0.6, 0.6), (0.0, 0.0, 0.0)))
ln2_ab_3=ListedColormap((
    (0.17647058823529413, 0.44313725490196076, 0.7137254901960784), # blue
    (0.33725490196078434, 0.6980392156862745, 0.4745098039215686), # green
    (0.8549019607843137, 0.21568627450980393, 0.19215686274509805), # red
    (0.9803921568627451, 0.9294117647058824, 0.3333333333333333), # yellow
    (0.4627450980392157, 0.30980392156862746, 0.396078431372549), # purple
    (0.6, 0.6, 0.6), # gray
    (0.0, 0.0, 0.0) # black
)) # black
# lnstent=ListedColormap(((0.17647058823529413, 0.44313725490196076, 0.7137254901960784), (0.8549019607843137, 0.21568627450980393, 0.19215686274509805), (0.8549019607843137, 0.21568627450980393, 0.19215686274509805), (0.33725490196078434, 0.6980392156862745, 0.4745098039215686), (0.4627450980392157, 0.30980392156862746, 0.396078431372549), (0.6, 0.6, 0.6), (0.0, 0.0, 0.0)))

# colors = [(0.17647058823529413, 0.44313725490196076, 0.7137254901960784), (1,1,1), (0.8549019607843137, 0.21568627450980393, 0.19215686274509805)]
# positions=[0, 0.5, 1]
fstabr=LinearSegmentedColormap.from_list("fstabr", list(zip([0, 0.5, 1], [(0.17647058823529413, 0.44313725490196076, 0.7137254901960784), (1,1,1), (0.8549019607843137, 0.21568627450980393, 0.19215686274509805)])))
    ...
    if overlap is True:
        ...
        for df_2col in ls_df_2col:
            if type(df_2col) is pd.DataFrame:
                ...
                data = pd.read_csv(df_2col, **read_csv_kwargs)
...
def preview_2coldf_group_v2(
    ...
    # if showninwn is True:
    #     for df_2col in ls_df_2col:
    #         df_2col.iloc[:, 0::2] = 1e7 / df_2col.iloc[:, 0::2]

    if mf.compare_column_counts(*ls_df_2col):
        num_group = ls_df_2col[0].shape[1] // 2
        for i in range(num_group):
            group = "_".join(ls_df_2col[0].columns[2 * i + 1].split("_")[:2])
            color = colorway[i]
            for df_2col in ls_df_2col:
                data = df_2col.iloc[:, 2 * i : 2 * i + 2]
                if showninwn is True:
                    data.iloc[:, 0] = 1e7 / data.iloc[:, 0]
                    fig.update_xaxes(autorange="reversed")
                fig.add_scatter(
                    x=data.iloc[:, 0],
                    y=data.iloc[:, 1],
...
def display_ab_wn_wl(
    ...
    if xlimit1[0] is not None:
        ...
        fig.update_layout(xaxis2_range=[10**4 / xlimit1[0][0], 10**4 / xlimit1[0][1]])
...
def display_pl_bwn_twl(
    ...
    if fignum is not None:
        ax.text(-0.14, 1.14, fignum, transform=ax.transAxes, fontsize=8, verticalalignment='top', horizontalalignment='left')
    return fig
...
def display_ex_bwn_twl(
    ...
    if df_1cols is not None:
        df_2cols = mf.reshape_1colto2col_v2(df_1cols)
    else:
        df_2cols = df_2cols

    num_sets = int(len(df_2cols.columns) / 2)
    for i in range(0, num_sets):
        df_2col = df_2cols.iloc[:,[2*i, 2*i+1]]

        if legendtext is None:
            if format_legendtext is None:
                label = df_2col.columns[1]
            else:
                label = format_legendtext(df_2col.columns[1])
        else:
            label = legendtext[i]
    ...
    ax.set_xlabel("Excitation Wavenumber (\u00d710$^3$ cm$^{-1}$)", fontsize=8, labelpad=2)
    if norm is True:
        ax.set_ylabel("Norm. PL Intensity", fontsize=8, labelpad=2)
    else:
        ax.set_ylabel("PL Intensity (Counts)", fontsize=8, labelpad=2)
    ...
    secax = ax.secondary_xaxis("top", functions=(mf.e3cm1_to_nm, mf.nm_to_e3cm1))
    secax.set_xlabel("Excitation Wavelength (nm)", fontsize=8, labelpad=3)
    secax.xaxis.set_major_formatter("{x:.0f}")
    secax.tick_params(axis="x", which="major", labelsize=6, width=0.5, length=2, pad=1)
    secax.tick_params(axis="x", which="minor", labelsize=6, width=0.5, length=1, pad=1)
...
def display_abpl_bwn_twl(
    ...
    if titletext is not None:
        ax1.set_title(titletext, fontsize=8, pad=2, loc=titleposition)
    ...
    # ax1.annotate('Annotation', xy=(3, 5), xytext=(4, 8),
    #         arrowprops=dict(facecolor='black', shrink=0.05))
    if fignum is not None:
        # ax1.text(-0.14, 1.14, fignum, transform=ax1.transAxes, fontsize=8, verticalalignment='top', horizontalalignment='left')
        ax1.text(0.02, 0.975, fignum, transform=ax1.transAxes, fontsize=8, verticalalignment='top', horizontalalignment='left')
    return fig
...
def display_nspl_2colkinetics_fitted_linear(
    ...
    for i, df_2cols in enumerate(ls_df_2cols):
        ...
        if xshift > 0:
            df_copy.iloc[:, [0, 2]] = df_copy.iloc[:, [0, 2]] - xshift
        elif xshift < 0:
            df_copy.iloc[:, [0, 2]] = df_copy.iloc[:, [0, 2]] + xshift
        ...
        if xscale == 0:
            ...
            df_copy.iloc[:, [0, 2]] = df_copy.iloc[:, [0, 2]] / 1000
            ax.set_xlabel("Time (us)", fontsize=8, labelpad=2)
        elif xscale == 6:
            df_copy.iloc[:, [0, 2]] = df_copy.iloc[:, [0, 2]] / 1000000
            ax.set_xlabel("Time (ms)", fontsize=8, labelpad=2)
        elif xscale == 9:
            df_copy.iloc[:, [0, 2]] = df_copy.iloc[:, [0, 2]] / 1000000000
            ax.set_xlabel("Time (s)", fontsize=8, labelpad=2)

        if ynorm is True:
            # ax.set_ylabel("Norm. PL Intensity", fontsize=8, labelpad=2)
            ax.set_ylabel("Norm. PL Intensity (a.u.)", fontsize=8, labelpad=2)
        else:
            if yscale == 0:
                # ax.set_ylabel("PL Intensity (Counts)", fontsize=8, labelpad=2)
                ax.set_ylabel("PL Intensity (a.u.)", fontsize=8, labelpad=2)
            else:
                df_copy.iloc[:, [1, 3]] = df_copy.iloc[:, [1, 3]] / 10**yscale
                ax.set_ylabel(
                    # "PL Intensity (\u00d710$^{" + str(yscale) + "}$ Counts)",
                    "PL Intensity (\u00d710$^{" + str(yscale) + "}$ a.u.)",
                    fontsize=8,
                    labelpad=2,
                )
...
def display_nspl_2colkinetics_fitted_symlog(
    ls_df_2cols,  # df_2cols with 4 columns, which represents the raw data, fitted data
    xshift=0,
    yshift=0,
    xscale=0,
    yscale=0,
    ...
    xticks = mf.determine_ns_timedelay_label(xlimit)
    ...
    ax.tick_params(axis="both", which="major", labelsize=6, width=0.5, length=2, pad=1)
    ax.tick_params(axis="both", which="minor", labelsize=6, width=0.5, length=1, pad=1)
    ax.ticklabel_format(axis="x", style="plain", scilimits=(5, 5), useMathText=True)
...
def display_fsta_contourf_log_bnm(
    df_fsta,
    xlimit=[[350, 700], 50, 25],
    ylimit=[[0.1, 10000], None, None],
    zlimit=[[-10, 10], 2, 1],
...
def display_fsta_contourf_log_be3cm1(
    df_fsta,
    xlimit=[[350, 700], 50, 25],
    ylimit=[[0.1, 10000], None, None],
    zlimit=[[-10, 10], 2, 1],
...
def display_fsta_fitted_spectra_bnm(
    ...
    if list_df[0].shape[1] != list_df[1].shape[1]:
        ...
        ax.tick_params(
            axis="both", which="minor", labelsize=6, width=0.5, length=1, pad=1
        )
        for axis in ["top", "bottom", "left", "right"]:
            ax.spines[axis].set_linewidth(0.5)
        if titletext is not None:
...
def display_fsta_1colkinetics_fitted_symlog(
    ...
    if fignum is not None:
        # ax.text(-0.1, 1.1, fignum, transform=ax.transAxes, fontsize=8, verticalalignment='top', horizontalalignment='left')
        ax.text(0.02, 0.975, fignum, transform=ax.transAxes, fontsize=8, verticalalignment='top', horizontalalignment='left')
    return fig
