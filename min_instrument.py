...
def ab_merge(filepaths):
    merged_ab_data = pd.DataFrame()
    ...
    for i in range(file_amounts):
        ...
        current_data = pd.read_csv(
            ...
            sep=",",
            skiprows=2,
            names=[f"ab_wl_nm", f"{filename}"],
            usecols=[0, 1],
        )
        merged_ab_data = pd.concat([merged_ab_data, current_data], axis=1)
    return merged_ab_data
